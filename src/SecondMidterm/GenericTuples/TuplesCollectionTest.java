package SecondMidterm.GenericTuples;
...
class TuplesCollection<T extends Comparable<T>>{
    ...
    public TreeSet<Tuple2<T>> getFilteredTuples(Predicate<Tuple2<T>> predicate) {
        return tuples.stream()
                .filter(predicate)
                .collect(Collectors.toCollection(() -> new TreeSet<Tuple2<T>>()));
    }
    ...
    int compare(TuplesCollection<? extends Comparable<?>> other) {
        return Integer.compare(this.tuples.size(), other.tuples.size());
    }
}
...
class TuplesBuilder {
    ...
    public static Tuple2<Double> buildDoublesTuple(Double double1, Double double2) {
