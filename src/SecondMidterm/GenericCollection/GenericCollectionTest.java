package SecondMidterm.GenericCollection;
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ...
    private TreeMap<LocalDateTime, T> elementsByDate;
    private HashMap<String, TreeSet<T>> elementsByCategory;
    ...
    public void addGenericItem(String category, T element) {
        ...
        elementsByCategory.computeIfPresent(category, (k, v) -> {v.add(element); return v;});
        elementsByDate.put(element.getTimestamp(), element);
    }

    public Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to) {
        TreeSet<T> tmpSet = new TreeSet<>(elementsByDate.subMap(from, to)
                .values());
        TreeSet<T> finalSet = new TreeSet<>(Comparator.reverseOrder());
        finalSet.addAll(tmpSet);
        return finalSet;
    }

    public Collection<T> itemsFromCategories (List<String> categories) {
        TreeSet<T> tmpSet = categories.stream()
...
//        return elementsByDate.entrySet()                  ne rabote na code poradi kompajler
//                .stream()
//                .collect(Collectors.groupingBy(
//                        (entry -> String.format("%02d-%d", entry.getKey().getMonthValue(), entry.getKey().getDayOfMonth())),
//                        TreeMap::new,
//                        (Collectors.mapping(entry -> entry.getValue(), Collectors.toCollection(() -> new TreeSet<>(Comparator.reverseOrder()))))
//                ));


        return elementsByDate.entrySet()
                        .stream()
                        .collect(Collectors.toMap(
                                (entry -> String.format("%02d-%02d", entry.getKey().getMonthValue(), entry.getKey().getDayOfMonth())),
                                entry -> {
                                    TreeSet<T> ts = new TreeSet<>(Comparator.reverseOrder());
