...
class Scheduler<T>{
    private TreeMap<Date, T> valuesByDate;
    ...
    public void add(Date date, T t) {
        valuesByDate.putIfAbsent(date, t);
    }


    public boolean remove(Date d) {
        if(valuesByDate.containsKey(d)){
            valuesByDate.remove(d);
            return true;
        }
        else return false;
    }
    ...
    public T next() {
        return valuesByDate.ceilingEntry(new Date()).getValue();
    }

    public T last() {
        return valuesByDate.floorEntry(new Date()).getValue();
    }


    public ArrayList<T> getAll(Date begin, Date end) {
        return new ArrayList<>(valuesByDate.subMap(begin, false, end, false).values());
    }

    public T getFirst() {
        return valuesByDate.get(valuesByDate.firstKey());
    }

    public T getLast() {
        return valuesByDate.lastEntry().getValue();
    }
}
