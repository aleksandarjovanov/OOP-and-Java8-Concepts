...
interface PatientService<T extends Patient> {
        List<T> patientsInformation(ArrayList<T> patients, String filter);      // ArrayList i go smenih so List
}
...
class HospitalSystem<T extends Patient & Comparable<T>> {
    ...
    public void printResults() {
        ...
        PatientService<T> findAllWithDoctor = (patients, filter) -> patients.stream().filter(patient -> patient.getDoctor().equals(filter)).collect(Collectors.toList());

        PatientService<T> countAllWithGenderAndAgeLessThanSixty = (patients, filter) -> patients.stream()
                .filter(patient -> ageAndGender(patient, filter)).collect(Collectors.toList());
    ...
    private boolean ageAndGender(Patient p, String filter) {
        return p.getAge() < 60 && p.getGender().equals(filter);
    }
}
